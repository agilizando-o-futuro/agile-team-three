...
3. Verifique se uma palavra é palíndromo (lê-se igual de trás pra frente)
   ...
   console.log(palavra + " é um palíndromo? " + (palavra === inverter(palavra)));

4. Encontre o maior número em um array

   const pontos = [10, 50, 55, 60, 23, 54, 100];

   function extrairResultado(pontos) {
   let maiorResultado = pontos[0];
   for (let i = 0; i < pontos.length; i++){
   let numeroDaVez = pontos[i];
   if(numeroDaVez > maiorResultado){
   maiorResultado = numeroDaVez;
   }
   }
   return maiorResultado;
   }
   console.log(extrairResultado(pontos));
...
1. Escreva no papel o algoritmo para trocar um pneu furado
   ...
   Com a chave de roda, afrouxar os parafusos da roda furada.

   Posicionar o macaco no ponto de apoio do chassi próximo da roda furada.

   Acionar o macaco até que o pneu furado saia totalmente do chão.

   Retirar completamente os parafusos da roda.

   Remover o pneu furado.

   Encaixar o estepe na posição correta.

   Colocar os parafusos e rosqueá-los até o limite sem uso de muita força.

   Abaixar o macaco até que o carro toque firmemente o chão e retirá-lo.

   Apertar os parafusos em padrão cruzado com a chave de roda.
...
2. Desenhe o fluxograma de uma calculadora de média escolar
   ...
   dividir pelo número de atividades -->
   resultado da média -->
   FIM
